...
def test_validation_passes_when_input_contains_only_asset_id(
    asset_valuation_argument_validator_with_parameters,
):
    asset_valuation_argument_validator_with_parameters.validate()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "GOOG:",
    ],  # Main component separator is present but period dates are missing
)
@pytest.mark.parametrize("period_format", ["%Y/%m/%d"])
